...
private:
	...
	bool place_mines(int color, int amount);
	...
	bool place_head(int color, int amount);
	bool place_mushroom(int color, int amount);
	bool place_ghost(int color, int amount);
	bool place_pipe(int color, int amount,int shape);
	void DebugLog(int i);
	std::set<Point> get_region_points(Point pos);
	std::vector<int> get_region_grid_patterns(std::set<Point> points);
	bool place_antitriangle(int color, int amount);
	bool check_it_is_corner(Point pos);
	bool place_dart(int color, int amount);
	bool place_raindrop(int color, int amount);
	bool isSurrounded(Point pos, Point dir, int type);
	bool place_pointer(int color, int amount);
